...
class RoleAction extends RoleService
{
       ...
       {
              return API::success($this->prepare($model, $role)->handler(Config::ASSIGN_USER), "succes asssign this user for given role", Response::HTTP_CREATED);
       }
       /**
       ...
       {
              return API::success($this->prepare($model, $role)->handler(Config::REMOVE_USER), "succes remove this user for given role", Response::HTTP_OK);
       }
       /**
       ...
       {
              return API::success($this->prepare($model, $role)->handler(Config::SYNCHRONIZE_USER), "Succes Sync User", Response::HTTP_OK);
       }
}
